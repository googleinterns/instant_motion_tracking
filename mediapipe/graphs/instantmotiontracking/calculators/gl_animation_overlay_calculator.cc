...
//   GIF_TEXTURE (AssetTextureFormat, required):
//     Will load a dynamic texture that is overlayed upon the GIF_ASSET .obj file
//
// Input side packets:
//   ROBOT_TEXTURE (ImageFrame on Android / GpuBuffer on iOS, required):
//     Texture to use for the Robot animation
//   ROBOT_ASSET (String, required):
//     Path of animation file to load and render for the Robot animation. Should
//     be generated by SimpleObjEncryptor.jar with --compressed_mode=true.
//   DINO_TEXTURE (ImageFrame on Android / GpuBuffer on iOS, required):
//     Texture to use with the Dino animation file.
//   DINO_ASSET (String, required):
//     Path of animation file to load and render for the Dino animation. Should
//     be generated by SimpleObjEncryptor.jar with --compressed_mode=true.
//   GIF_ASSET (String, required):
//     Path of animation file (.obj.uuu) that will be used to texture a GIF overlay
...
class GlAnimationOverlayCalculator : public CalculatorBase {
 ...
 private:
  ...
  GlTexture robot_texture_, dino_texture_, gif_texture_;
  std::vector<TriangleMesh> robot_triangle_meshes_, dino_triangle_meshes_, gif_triangle_meshes_;
...
::mediapipe::Status GlAnimationOverlayCalculator::GetContract(
  ...
  if (cc->Inputs().HasTag("GIF_TEXTURE")) {
    cc->Inputs().Tag("GIF_TEXTURE").Set<AssetTextureFormat>();
  }
  ...
  cc->InputSidePackets().Tag("GIF_ASSET").Set<std::string>();
...
::mediapipe::Status GlAnimationOverlayCalculator::Open(CalculatorContext *cc) {
  ...
  const std::string &gif_asset_name = cc->InputSidePackets().Tag("GIF_ASSET").Get<std::string>();
  bool loaded_animation = false;

  #if defined(__ANDROID__)
    loaded_animation = LoadAnimationAndroid(gif_asset_name, &gif_triangle_meshes_);
  ...
  #else
    loaded_animation = LoadAnimation(gif_asset_name);
...
::mediapipe::Status GlAnimationOverlayCalculator::Process(
  ...
  return helper_.RunInGlContext([this, &cc]() -> ::mediapipe::Status {
    ...
    TriangleMesh &gif_current_frame = gif_triangle_meshes_[0];

    // Load in the latest GIF frame for dynamic texturing
    if (cc->Inputs().HasTag("GIF_TEXTURE")) {
      const auto &input_gif_texture =
          cc->Inputs().Tag("GIF_TEXTURE").Get<AssetTextureFormat>();
      gif_texture_ = helper_.CreateSourceTexture(input_gif_texture);
    }
